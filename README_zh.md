...
从 [Releases](https://github.com/ffflyZzz/octopus/releases) 下载对应平台的二进制文件，然后运行：
...
- 当创建渠道时，若渠道包含的模型不在 models.dev 中，系统会自动创建该模型的价格信息，用户可以手动设置价格
...
| 优先级 | 来源 | 说明 |
|:------:|------|------|
| 🥇 高 | 渠道专属价格 | 为特定渠道-模型组合配置的价格 |
| 🥈 中 | 默认价格 | 在价格管理页面设置的用户自定义默认价格 |
| 🥉 低 | models.dev | 从上游自动同步的默认价格 |

**核心功能：**

- 🎯 **按渠道组织** - 通过渠道标签页查看和管理模型
- 💱 **独立定价** - 为不同渠道下的相同模型设置不同价格
- 🔄 **自动费用计算** - 系统在计算请求费用时自动使用渠道专属价格
- 📊 **分渠道统计** - 为每个渠道单独追踪 Token 使用量和费用

> 💡 **示例**：您可以在渠道 A 将 `claude-sonnet-4` 设置为 $3.00/$15.00，在渠道 B 设置为 $2.50/$12.00，系统会为每个渠道准确计算费用。

> 💡 **提示**：要为特定渠道的模型设置价格，请在价格管理页面切换到该渠道的标签页，然后编辑模型价格。
